...
const getComment = async () => {
  try {
    const response = await axios.get(`https://xyz.waguwagu.online/api/comments/${postId}`);
...
const createComment = async () => {
  try {
    ...
    const response = await axios.post(`https://xyz.waguwagu.online/api/comments/${postId}`, { content }, headers);
...
const updateComment = async (commentId) => {
  try {
    ...
    await axios.put(`https://xyz.waguwagu.online/api/comments/${commentId}`, { content }, headers);
  } catch (error) {
...
const deleteComment = async (commentId) => {
  try {
    await axios.delete(`https://xyz.waguwagu.online/api/comments/${commentId}`, headers);
    location.reload();
  } catch (error) {
    console.log(error);
    alert(error.response.data.data);
  ...
  $('#updatePost')
    ...
    .on('click', async function (event) {
      ...
      if (userConfirmed) {
        ...
        try {
          const response = await axios.patch(`https://xyz.waguwagu.online/api/posts/${postId}`, formData, {
            headers: {
...
const reportPost = async () => {
  try {
    ...
    await axios.post(
      `https://xyz.waguwagu.online/api/posts/${postId}/reports`,
      { report_type: $('#report-type').val(), reported_reason: $('.input-report').val() },
      headers,
    );
...
const deletePost = async () => {
  try {
    await axios.delete(`https://xyz.waguwagu.online/api/posts/${postId}`, headers);
    location.reload();
  } catch (error) {
    console.log(error.response);
    alert(error.response.data.errorMessage);
